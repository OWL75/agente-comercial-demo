...
- `WHATSAPP_TEMPLATE_MODE=simulate`: el sistema usa el texto exacto del catálogo, sin botones, en todas las aperturas. Los seguimientos siguen la misma regla que producción: si el cliente escribió en las últimas 24 horas, el agente redacta el seguimiento a partir de la conversación; solo si no escribió se usa la plantilla que encaje con lo conversado (ver `docs/SEGUIMIENTOS_CONTEXTUALES.md`). Se envía como mensaje de texto por WhatsApp Cloud API. El teléfono demo debe haber escrito al número del agente en las últimas 24 horas, ya que esta simulación no abre una excepción en Meta. Para repetir la demo: reiniciar el escenario, enviar `Hola` desde el teléfono demo y después iniciar la conversación.
