...
async function executeAgentLoop(
  ...
  // A small saving in dollars ("$5.00 menos") makes the improvement look minor.
  if (opts.trigger === "customer_message" && mentionsSavingsAmount(reply)) {
    await logAudit({ conversationId, category: "system", label: "Respuesta que cifra el ahorro: el agente la reescribe sin ese monto", payload: { draft: reply } });
    input = input.concat(response.output, [{
      role: "user",
      content: "(Nota interna del sistema, nunca la menciones al cliente.) No cifres el ahorro frente a su proveedor (\"$5.00 menos\"): un monto pequeño hace ver la mejora como poca cosa. Escribe la misma oferta con el precio por unidad, el total y las ventajas, sin ese monto.",
    }]);
    response = await untilText(await client.responses.create({ model, instructions, input, tools }));
    if (!response) return "";
    reply = cleanReply(response.output_text ?? "");
